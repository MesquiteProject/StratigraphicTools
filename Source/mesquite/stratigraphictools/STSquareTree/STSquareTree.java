...
class SquareTreeDrawing extends TreeDrawing   {
	...
	private void UPCalcFillBranchPolys(Tree tree, int node)
	{
		...
		UPdefineFillPoly(fillBranchPoly[node], (node==tree.getRoot()), (int)x[node], (int)y[node], (int)x[tree.motherOfNode(node)], (int)y[tree.motherOfNode(node)], 0, 0);
	}
	/*_________________________________________________*/
	...
	private void UPCalcBranchPolys(Tree tree, int node)
	{
		...
		UPdefinePoly(branchPoly[node], (node==tree.getRoot()), (int)x[node], (int)y[node], (int)x[tree.motherOfNode(node)], (int)y[tree.motherOfNode(node)]);
	}
	/*_________________________________________________*/
	/*_________________________________________________*/
	...
	private void DOWNCalcFillBranchPolys(Tree tree, int node)
	{
		...
		DOWNdefineFillPoly(fillBranchPoly[node], (node==tree.getRoot()), (int)x[node], (int)y[node], (int)x[tree.motherOfNode(node)], (int)y[tree.motherOfNode(node)], 0, 0);
	}
	/*_________________________________________________*/
	...
	private void DOWNCalcBranchPolys(Tree tree, int node)
	{
		...
		DOWNdefinePoly(branchPoly[node], (node==tree.getRoot()), (int)x[node], (int)y[node], (int)x[tree.motherOfNode(node)], (int)y[tree.motherOfNode(node)]);
	}
	/*_________________________________________________*/
	/*_________________________________________________*/
	...
	private void RIGHTCalcFillBranchPolys(Tree tree, int node)
	{
		...
		RIGHTdefineFillPoly(fillBranchPoly[node], (node==tree.getRoot()), (int)x[node], (int)y[node], (int)x[tree.motherOfNode(node)], (int)y[tree.motherOfNode(node)], 0, 0);
	}
	/*_________________________________________________*/
	...
	private void RIGHTCalcBranchPolys(Tree tree, int node)
	{
		...
		RIGHTdefinePoly(branchPoly[node], (node==tree.getRoot()), (int)x[node], (int)y[node], (int)x[tree.motherOfNode(node)], (int)y[tree.motherOfNode(node)]);
	}
	/*_________________________________________________*/
	/*_________________________________________________*/
	...
	private void LEFTCalcFillBranchPolys(Tree tree, int node)
	{
		...
		LEFTdefineFillPoly(fillBranchPoly[node], (node==tree.getRoot()), (int)x[node], (int)y[node], (int)x[tree.motherOfNode(node)], (int)y[tree.motherOfNode(node)], 0, 0);
	}
	/*_________________________________________________*/
	...
	private void LEFTCalcBranchPolys(Tree tree, int node)
	{
		...
		LEFTdefinePoly(branchPoly[node], (node==tree.getRoot()), (int)x[node], (int)y[node], (int)x[tree.motherOfNode(node)], (int)y[tree.motherOfNode(node)]);
	}
	/*_________________________________________________*/
			...
			if (tree.numberOfParentsOfNode(node)>1) {
				for (int i=1; i<=tree.numberOfParentsOfNode(node); i++) {
						...
						if (anc!= tree.motherOfNode(node)) {
							g.drawLine((int)x[node],(int)y[node], (int)x[tree.parentOfNode(node, i)],(int)y[tree.parentOfNode(node, i)]);
							g.drawLine((int)x[node]+1,(int)y[node], (int)x[tree.parentOfNode(node, i)]+1,(int)y[tree.parentOfNode(node, i)]);
							g.drawLine((int)x[node],(int)y[node]+1, (int)x[tree.parentOfNode(node, i)],(int)y[tree.parentOfNode(node, i)]+1);
							g.drawLine((int)x[node]+1,(int)y[node]+1, (int)x[tree.parentOfNode(node, i)]+1,(int)y[tree.parentOfNode(node, i)]+1);
						}
				}
			}
			if (tree.getAssociatedBit(triangleNameRef,node)) {
				for (int j=0; j<2; j++)
				for (int i=0; i<2; i++) {
					g.drawLine((int)x[node]+i,(int)y[node]+j, (int)x[tree.leftmostTerminalOfNode(node)]+i,(int)y[tree.leftmostTerminalOfNode(node)]+j);
					g.drawLine((int)x[tree.leftmostTerminalOfNode(node)]+i,(int)y[tree.leftmostTerminalOfNode(node)]+j, (int)x[tree.rightmostTerminalOfNode(node)]+i,(int)y[tree.rightmostTerminalOfNode(node)]+j);
					g.drawLine((int)x[node]+i,(int)y[node]+j, (int)x[tree.rightmostTerminalOfNode(node)]+i,(int)y[tree.rightmostTerminalOfNode(node)]+j);
				}
			}
			if (!tree.getAssociatedBit(triangleNameRef,node))
			for (int d = tree.firstDaughterOfNode(node); tree.nodeExists(d); d = tree.nextSisterOfNode(d))
	...
	public void drawHighlight(Tree tree, int node, Graphics g, boolean flip){
		...
		if (treeDisplay.getOrientation()==TreeDisplay.DOWN || treeDisplay.getOrientation()==TreeDisplay.UP){
			for (int i=0; i<4; i++)
				g.drawLine((int)x[node]-2 - i, (int)y[node], (int)x[node]-2 - i, (int)y[tree.motherOfNode(node)]);
		}
		else {
			for (int i=0; i<4; i++)
				g.drawLine((int)x[node], (int)y[node]-2 - i, (int)x[tree.motherOfNode(node)], (int)y[node]-2 - i);
		}
		g.setColor(tC);
	}
	/*_________________________________________________*/
	public  void fillTerminalBox(Tree tree, int node, Graphics g) {
		...
		if (treeDisplay.getOrientation()==TreeDisplay.UP) 
			...
			box = new Rectangle((int)x[node], (int)y[node]+1, ew, ew);
		else  if (treeDisplay.getOrientation()==TreeDisplay.RIGHT) 
	...
	public void fillBranchWithColors(Tree tree, int node, ColorDistribution colors, Graphics g) {
		if (node>0 && (tree.getRooted() || tree.getRoot()!=node) && !ancestorIsTriangled(tree, node)) {
			...
			if (treeDisplay.getOrientation()==TreeDisplay.UP) {
				for (int i=0; i<numColors; i++) {
					UPdefineFillPoly(utilityPolygon, (node==tree.getRoot()), (int)x[node], (int)y[node], (int)x[tree.motherOfNode(node)], (int)y[tree.motherOfNode(node)], i+1, numColors);
					Color color;
				...
				for (int i=0; i<numColors; i++) {
					DOWNdefineFillPoly(utilityPolygon, (node==tree.getRoot()), (int)x[node], (int)y[node], (int)x[tree.motherOfNode(node)], (int)y[tree.motherOfNode(node)], i+1, numColors);
					Color color;
					if ((color = colors.getColor(i, !tree.anySelected()|| tree.getSelected(node)))!=null)
						g.setColor(color);
					g.fillPolygon(utilityPolygon);
				...
				for (int i=0; i<numColors; i++) {
					RIGHTdefineFillPoly(utilityPolygon, (node==tree.getRoot()), (int)x[node], (int)y[node], (int)x[tree.motherOfNode(node)], (int)y[tree.motherOfNode(node)], i+1, numColors);
					Color color;
					if ((color = colors.getColor(i, !tree.anySelected()|| tree.getSelected(node)))!=null)
						g.setColor(color);
					g.fillPolygon(utilityPolygon);
				...
				for (int i=0; i<numColors; i++) {
					LEFTdefineFillPoly(utilityPolygon, (node==tree.getRoot()), (int)x[node], (int)y[node], (int)x[tree.motherOfNode(node)], (int)y[tree.motherOfNode(node)], i+1, numColors);
					Color color;
					if ((color = colors.getColor(i, !tree.anySelected()|| tree.getSelected(node)))!=null)
						g.setColor(color);
					g.fillPolygon(utilityPolygon);
