...
public abstract class ScaleDistribution  extends ScaleStates implements CharacterDistribution {
	
	public ScaleDistribution (Taxa taxa) {
		super(taxa);
	}
	...
	public AdjustableDistribution getAdjustableClone(){
		ScaleAdjustable soc = new ScaleAdjustable(getTaxa(), getNumTaxa());
		soc.setItemsAs(this);
	...
	public CharacterHistory adjustHistorySize(Tree tree, CharacterHistory charStates) {
		...
		if (charStates==null || ! (charStates.getClass() == ScaleHistory.class)) 
			soc = new ScaleHistory(tree.getTaxa(), numNodes, (ScaleData)getParentData()); //should keep items!!! (since StsOfChar may have different number from parent data!
		else if (numNodes!= charStates.getNumNodes()) 
			soc = new ScaleHistory(tree.getTaxa(), numNodes, (ScaleData)getParentData());
		else {
			soc =charStates;
		}
		soc.setParentData(getParentData());
