...
public class MScaleAdjustable  extends MScaleDistribution implements MAdjustableDistribution {
	int numTaxa=0;
	int numChars=0;
	protected Double2DArray firstMatrix; //other dimension for items (sample size, variance, etc., etc.)
	protected Vector matrices;
	String annotation;
	public MScaleAdjustable (Taxa taxa, int numChars, int numTaxa) {
		super(taxa);
	...
	public MScaleAdjustable (Taxa taxa) {
		super(taxa);
		matrices = new Vector(0);
	}
	public MScaleAdjustable () {
		super(null);
		matrices = new Vector(0);
	}
			...
			if (matrix !=null)
				for (int k=0; k<numChars; k++)
					for (int j=0; j<numTaxa; j++)
						matrix.setValue(k, j, ScaleState.unassigned); 
		}
	}
	...
	public void transferFrom(int ic, CharacterDistribution s) { 
		if (s instanceof ScaleDistribution) {
			setItemsAs(((ScaleDistribution)s));
			for (int item=0; item<((ScaleDistribution)s).getNumItems(); item++) {
				Double2DArray matrix = getItem(item);
				if (matrix !=null)
					for (int j=0; j<getNumNodes(); j++)
						matrix.setValue(ic, j,  ((ScaleDistribution)s).getState(j, item));
			}
		}
	}
	...
	public CharacterDistribution getCharacterDistribution (int ic) {
		ScaleAdjustable soc = new ScaleAdjustable(getTaxa(), numTaxa);
		soc.setItemsAs(this);
		...
		if (getParentData()!=null)
			soc.setName("Time " + ic + " of " + getParentData().getName());
		else
			soc.setName("Time " + ic );
		return soc;
	}
